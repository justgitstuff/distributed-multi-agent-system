...
/**
 * Modified 20-01-16: added integration of self written class 'JobSeries'. 
...
public class Legend extends Composite implements ILegend, PaintListener {
    ...
    protected void drawSymbol(GC gc, Series series, Rectangle r) {
        ...
        if (series instanceof IJobSeries) {
            // draw plot line
            gc.setForeground(((IJobSeries) series).getLineColor());
            gc.setLineWidth(LINE_WIDTH);
            int lineStyle = Util.getIndexDefinedInSWT(((IJobSeries) series)
                    .getLineStyle());
            int x = r.x;
