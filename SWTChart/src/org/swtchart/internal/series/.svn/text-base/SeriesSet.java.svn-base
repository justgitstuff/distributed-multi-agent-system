 ...
 * A series container.
 * 
 * Modified 09-11-18: added integration of self written class 'JobSeries'. 
...
public class SeriesSet implements ISeriesSet {
    ...
    public ISeries createSeries(SeriesType type, String id) {
        ...
        } else if (type == SeriesType.JOB) { // added 09-11-18 (Matthias Huber)
        	series = new JobSeries(chart, identifier);
        } else {
            SWT.error(SWT.ERROR_INVALID_ARGUMENT);
            return null; // to suppress warning...
        }
