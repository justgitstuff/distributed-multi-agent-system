...
public class ProposalConverter {
	 ...
	 */
	//TODO: remove methods fromProposalWrapperArrayToProposalList and fromProposalWrapperToProposal
	//		because they are now in the ProposalWrapperConverter class
	 ...
	 */
 	private IProposal fromProposalWrapperToProposal(ProposalWrapper wrappedProposal) { 
		...
		if (null != wrappedProposal) {
            ...
            ArrayOfint[] wrappedStartTimes = wrappedProposal.getStartTimes();
			...
			for (int resIndex = 0; resIndex < wrappedProposal.getNumberOfResources(); resIndex++) {
				...
				int[] wrappedStartTimeArray = wrappedStartTimes[resIndex].getArray();
				for (int jobIndex = 0; jobIndex < wrappedProposal.getNumberOfJobs(); jobIndex++) {
					    startTimeArray.add(wrappedStartTimeArray[jobIndex]);
				}
	...
	private ProposalWrapper fromProposalToProposalWrapper(IProposal sourceProposal) {
		...
		if (null != sourceProposal) {
			wrappedProposal = new ProposalWrapper();
			wrappedProposal.setNumberOfJobs(sourceProposal.getPermutationSize());
			wrappedProposal.setNumberOfResources(sourceProposal.getNumberOfResourceStarttimes());
			ArrayOfint[] starttimes = new ArrayOfint[sourceProposal.getNumberOfResourceStarttimes()];
			...
			for (int resIndex = 0; resIndex < sourceProposal.getNumberOfResourceStarttimes(); resIndex++) {
				int[] resStarttimes = new int[sourceProposal.getPermutationSize()];
				for (int jobIndex = 0; jobIndex < sourceProposal.getPermutationSize(); jobIndex++) {
					try {
						resStarttimes[jobIndex] = sourceProposal.getStarttime(resIndex, jobIndex);
					} catch (StarttimeNotFoundException e) {
						// StartTimeNotFoundException - should not happen
						e.printStackTrace();
					}
				}
				starttimes[resIndex] = new ArrayOfint();
				starttimes[resIndex].setArray( resStarttimes );
			}
	...
	public boolean compareProposalWrapperWithProposal(ProposalWrapper proposalWrapper, IProposal proposalObj){
		if (proposalWrapper == null && proposalObj == null) {
			...
			ArrayOfint[] wrappedStartTimes = proposalWrapper.getStartTimes();
            for(int i=0; i<proposalWrapper.getNumberOfResources(); i++){
				int[] starttimePerResource = wrappedStartTimes[i].getArray();
